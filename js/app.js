...
class PortfolioApp {
  ...
  discoverGroupNames() {
    const deleted = this.state.deleted;
    const accountEntries = this.state.imported.map((l, n) => ["Imported — " + l.file, "i" + n])
      .concat(this.state.custom.map((l, n) => [l.group, "c" + n]));
    const seedEntries = SEED.map((r, n) => [r[0], "s" + n]);
    const names = [];
    accountEntries.forEach(([g, key]) => { if (!deleted[key] && names.indexOf(g) < 0) names.push(g); });
    this.state.extraGroups.forEach(g => { if (names.indexOf(g) < 0) names.push(g); });
    seedEntries.forEach(([g, key]) => { if (!deleted[key] && names.indexOf(g) < 0) names.push(g); });
    return names;
  }
  ...
  buildViewModel() {
    ...
    const seedGroupSet = {};
    SEED.forEach(r => { seedGroupSet[r[0]] = true; });
    const groupNames = [];
    all.forEach(l => { if (!seedGroupSet[l.group] && groupNames.indexOf(l.group) < 0) groupNames.push(l.group); });
  ...
  <section class="no-print">
    ...
    <div class="connector">
      ...
      <div class="key-row">
        ...
        <input type="text" class="field key-input" value="${escAttr(vm.newSectionDraft)}" data-action="new-section-draft" placeholder="Account name, e.g. Fidelity IRA">
        <button class="btn-gold" data-action="add-section">+ Add account</button>
      </div>
