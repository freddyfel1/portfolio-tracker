...
class PortfolioApp {
  ...
  editSellPrice(key, text) {
    const n = num(text);
    if (isFinite(n) && n >= 0) {
      this.state.edits[key] = Object.assign({}, this.state.edits[key], { sellPrice: n });
      delete this.state.drafts["sp" + key];
      this.persist();
    } else {
      this.state.drafts["sp" + key] = text;
    }
    this.render();
  }
  editStopLoss(key, text) {
    const n = num(text);
    if (isFinite(n) && n >= 0) {
      this.state.edits[key] = Object.assign({}, this.state.edits[key], { stopLoss: n });
      delete this.state.drafts["sl" + key];
      this.persist();
    } else {
      this.state.drafts["sl" + key] = text;
    }
    this.render();
  }
  editTargetPrice(key, text) {
    const n = num(text);
    if (isFinite(n) && n >= 0) {
      this.state.edits[key] = Object.assign({}, this.state.edits[key], { targetPrice: n });
      delete this.state.drafts["tp" + key];
      this.persist();
    } else {
      this.state.drafts["tp" + key] = text;
    }
    this.render();
  }
  ...
  buildViewModel() {
    ...
    const all = SEED.map((r, n) => r.concat(["s" + n])).concat(importedLots).concat(customLots)
      ...
      .map(r => {
        ...
        const stopLoss = e.stopLoss !== undefined ? e.stopLoss : null;
        ...
        return { key, group, ticker, sub, qty, buy, buyDate, sellDate, sellPrice, stopLoss, targetPrice, price, has, edited, cost, usingFeed, feedAt: feed ? feed.at : "", feedSource: feed ? feed.source : "", pinned: !!this.state.pinned[ticker], value: has ? qty * price : null, pl: has ? qty * price - cost : null };
      });
    ...
    const groups = orderedNames.filter(g => {
      ...
      return {
        ...
        rows: gl.map(l => ({
          ...
          stopLossInput: this.state.drafts["sl" + l.key] !== undefined ? this.state.drafts["sl" + l.key] : (l.stopLoss !== null ? priceMoney(l.stopLoss) : ""),
...
<div class="page"><div class="wrap">
  ...
  <section class="group">
    ...
    <div class="cols col-head">
      ...
      <div class="right">Price now</div><div class="right">Stop Loss</div><div class="right">Target Price</div><div class="right">Sell Date</div><div class="right">Sell Price</div><div class="right">Value</div><div class="right">Sell Value</div><div class="right">P/L</div><div class="right">Return</div><div style="text-align:center;">Del</div>
    </div>
    ...
    <div class="cols lot-row">
      ...
      <div class="lot-input-cell">
        <input type="text" class="lot-input" value="${escAttr(row.stopLossInput)}" title="Price at which you'd exit to cut losses" data-action="edit-stop-loss" data-key="${escAttr(row.key)}">
      </div>
      <div class="lot-input-cell">
  ...
  <section class="notes">
    ...
    <ul>
      ...
      <li>Buy Date, Sell Date, Sell Price, Stop Loss, and Target Price are optional manual record-keeping fields per lot; Sell Value is simply qty × Sell Price. None of them affect cost basis, current value, or any total — nothing is auto-marked "sold" and no alert fires when price crosses Stop Loss or Target Price.</li>
    </ul>
  </section>
</div></div>`;
}
...
PortfolioApp.prototype.attachEvents = function () {
  ...
  root.addEventListener("change", e => {
    ...
    switch (action) {
      ...
      case "edit-stop-loss": this.editStopLoss(el.dataset.key, el.value); break;
